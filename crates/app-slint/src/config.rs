...
use api::trait_rpc::serde::{Deserialize, Serialize};
...
impl Config {
    ...
    pub async fn load() -> Self {
        ...
        let mut config = Self::load_file().await.unwrap_or_else(|err| {
    ...
    async fn load_file() -> anyhow::Result<Self> {
        let config_file = Self::open_file().await.context("Failed to open config file")?;
        let config = fs::read(config_file).await.context("Failed to read config file")?;
        if config.is_empty() {
    ...
    async fn save_file(&self) -> anyhow::Result<()> {
        ...
        fs::write(&config_file, config).await.context("Failed to write config file")?;
    ...
    async fn open_file() -> anyhow::Result<PathBuf> {
        ...
        if !config_dir.exists() {
            create_dir(&config_dir).await.context("Failed to create config directory")?;
        ...
        if !config_dir.exists() {
            create_dir(&config_dir).await.context("Failed to create app config directory")?;
        } else if !config_dir.is_dir() {
            return Err(anyhow!("App config directory is not a directory"));
        }
        let config_file = config_dir.join("config.json");
        if !config_file.exists() {
            File::create(&config_file).await.context("Failed to create app config file")?;
